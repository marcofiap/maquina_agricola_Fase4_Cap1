...
#define BUTTON1 2
#define BUTTON2 18
#define BUTTON_DESLIGAR_BOMBA 15    
...
// Variável para controlar o estado da bomba
...
void loop() {
  ...
  String fosforo = (buttonPState == LOW) ? "true" : "false";          // Operador ternário -> condição ? valor_se_verdadeiro : valor_se_falso;
  String potassio = (buttonKState == LOW) ? "true" : "false";
  String f = fosforo;
  String p = potassio;
  ...
  // Lógica de controle do relé usando switch statement
  ...
  if (botaoDesligarState == LOW) {
    controleBombaEstado = 2; // Botão de desligar pressionado (manual OFF)
  } else if (mostrarIrrigacao) {
    controleBombaEstado = 1; // Umidade abaixo de 40% (automático ON)
  } else {
    controleBombaEstado = 0; // Umidade acima de 40% (OFF)
  }

  switch (controleBombaEstado) {
    case 0: // Umidade acima de 40% -> Desliga a bomba
